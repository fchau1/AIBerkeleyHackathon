class ReadingTracker {
    ...
    initializeGrid() {
      return Array(this.gridSize.rows).fill().map(() =>
        Array(this.gridSize.cols).fill().map(() => ({ count: 0, lastSeen: 0 }))
      );
    }
    ...
    updateGaze(x, y, timestamp) {
      ...
      if (row >= 0 && row < this.gridSize.rows && col >= 0 && col < this.gridSize.cols) {
        ...
        if (this.lastFocusedCell) {
          ...
          if (timestamp - this.lastFocusTime >= this.focusTime || row !== lastRow) {
